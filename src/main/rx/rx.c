...
#include "fc/runtime_config.h" //Added by Jake to get access to the flight mode during flight.
...
int16_t pilotInput[MAX_SUPPORTED_RC_CHANNEL_COUNT];
extern struct quadSasMessagePwmValues_ref *motorPWM;
extern int AUTO;
...
#endif
        {
			//This is where everything is happening. I need to add something in here to create the A/M Switch.
			switch (AUTO) {
			case 0: /*MANUAL MODE*/
				rcData[channel] = sample;
				if (!FLIGHT_MODE(ANGLE_MODE)) {
					ENABLE_FLIGHT_MODE(ANGLE_MODE); //when in manual, switch to angle mode.
				}
				break;
			case 1: /*AUTO MODE*/
				if (channel <= 3) {
					rcData[channel] = motorPWM->pwm[channel];
					if (FLIGHT_MODE(ANGLE_MODE)) {
						DISABLE_FLIGHT_MODE(ANGLE_MODE); //when in auto, switch to acro mode.
					}
				}
				/*passthrough the Auto/Manual and Arm/Disarm from transmitter*/
				else if ((channel > 3) && (channel < 7)) {
					rcData[channel] = sample;
				}
				else {
					rcData[channel] = 0;//GUST input only has 7 channels (avoid seg fault)
				}
				break;
			default: /*standard Betaflight course*/
				rcData[channel] = sample; 
				break;
			}
			pilotInput[channel] = sample; /*always save the pilot input in the same place*/
			//DEBUG_SET(DEBUG_NONE, 0, FLIGHT_MODE(ANGLE_MODE));
        }
    }
	//DEBUG_SET(DEBUG_NONE, 0, blah);
	//blah++;
	/*DEBUG_SET(DEBUG_NONE, 0, rcData[0]);
	DEBUG_SET(DEBUG_NONE, 1, rcData[1]);
	DEBUG_SET(DEBUG_NONE, 2, rcData[2]);
	DEBUG_SET(DEBUG_NONE, 3, rcData[3]);*/
